...
export async function GET(
  ...
  if (nna.length > 0) {
    ...
    const rango = (e: number) => (e <= 5 ? '0-5' : e <= 11 ? '6-11' : '12-17')
    const rangos = nna.map((n: any) => n.edad).filter((e: any) => e != null).map((e: number) => rango(e))
    lineas.push(`NNA involucrados: ${nna.length}${rangos.length ? ` (rangos etarios: ${rangos.join(', ')})` : ''}`)
  }
